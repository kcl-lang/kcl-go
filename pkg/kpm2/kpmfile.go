...
func (r *Require) NewRequireFromPkgString(pkgv string, gitflag bool) error {
	...
	if len(result) == 1 {
		...
		//如果是git包，直接拉取最新版
		if gitflag {
			tmp := os.TempDir() + Separator + Convert.B2S(Random.RandBytes32())
...
func (r *Require) PkgDownload(kpmroot, kpmserver string) error {
	...
	if r.Type == "git" {
		...
		for i := 0; i < len(pkginfo.Files); i++ {
			//检查本地是否有文件，如果没有，则下载
			fpath := kpmroot + Separator + "store" + Separator + "v1" + Separator + "files" + Separator + HashMod(Convert.S2B(pkginfo.Files[i].Integrity)) + Separator + pkginfo.Files[i].Integrity
			exists, err := PathExists(fpath)
			if err != nil {
				return err
			}
			if !exists {
				req.SetRequestURI("/s/store/v1/files/" + HashMod(Convert.S2B(pkginfo.Files[i].Integrity)) + "/" + pkginfo.Files[i].Integrity)
				resp.Reset()
				if err = fasthttp.Do(req, resp); err != nil {
					return err
				}
